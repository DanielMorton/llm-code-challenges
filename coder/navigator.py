...
class ProblemNavigator:
    ...
    def navigate_to_new_problem(self, df):
        current_page = 1
        while True:
            self.driver.navigate_to(f"{LEETCODEFILTER}{current_page}")
            time.sleep(5)
            self.wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, 'div[role="rowgroup"]')))

            for row in self.driver.find_elements(By.CSS_SELECTOR, 'div[role="row"]'):
                ...
                if len(cells) >= 2:
                    ...
                    if 'opacity-60' not in title_link.get_attribute('class') and title_link.text not in df[
                        'Problem Name']:
                        self.driver.navigate_to(title_link.get_attribute('href'))
                        time.sleep(5)
